...
export default function TripMap({stops}) {
  ...
  useEffect(()=>{
    ...
    stops.forEach((stop,index)=>{
      const popup=document.createElement('span');popup.textContent=stop.place.name;
      L.marker(points[index],{icon:L.divIcon({className:'number-pin',html:`<span>${index+1}</span>`,iconSize:[30,30],iconAnchor:[15,15]}),title:stop.place.name}).bindPopup(popup).addTo(map);
    });
  ...
  return <section className="map-card" aria-label="Map of itinerary stops"><div ref={container} className="trip-map"/><p>{tilesFailed?'Map tiles unavailable. Stops are listed in the timeline.':'OpenStreetMap · Dashed lines connect stops; they are not walking directions.'}</p></section>;
}
